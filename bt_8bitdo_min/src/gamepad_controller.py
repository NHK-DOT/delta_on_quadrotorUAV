...
class RealTimeArmController(object):
    ...
    def connect(self):
        ...
        try:
            ...
            print("Bluetooth gamepad opened.")
        except Exception as exc:
            print("Bluetooth gamepad open failed: %s" % exc)
            print("If this is a permission problem, run:")
            print("  bash deploy/install_ubuntu18.sh")
            print("then log out and log in again.")
            return False
        return True
    ...
    def build_status_snapshot(self):
        lines = [
            "bt_8bitdo_min runtime status",
            "time: %s" % datetime.now().isoformat(timespec="seconds"),
            "port: %s" % self.port,
            "gamepad_event_override: %s" % self.joystick_device,
...
def parse_args(argv):
    ...
    parser.add_argument("--joystick", default=DEFAULT_JOYSTICK, help="optional /dev/input/eventX override")
