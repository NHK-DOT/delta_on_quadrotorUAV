...
class ServoCalibrator:
    ...
    def move_all_servos(self, position: int, time_ms: int = 500) -> bool:
        ...
        try:
            ...
            for servo_id in self.servo_ids:
                min_pos, max_pos = self.servo_limits[servo_id]
                safe_pos = self.quantize_position(servo_id, position)
                safe_pos = max(min_pos, min(max_pos, safe_pos))
            ...
            for servo_id, pos in targets:
                coord = self.position_to_coord(servo_id, pos)
                print(f"  舵机 {servo_id}: raw {pos} (coord {coord:.2f})")
            time.sleep(time_ms / 1000.0 + 0.1)
            return True
        except Exception as e:
            print(f"✗ 命令失败: {e}")
