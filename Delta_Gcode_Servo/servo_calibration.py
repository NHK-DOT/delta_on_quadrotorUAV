...
class ServoCalibrator:
    ...
    def move_all_servos(self, position: int, time_ms: int = 500) -> bool:
        """同时移动所有舵机（注意舵机 2 的上限）"""
        if not self.driver.ser or not self.driver.ser.is_open:
            print("错误: 串口未打开")
            return False
        ...
        try:
            ...
            for servo_id in [1, 2, 3]:
                min_pos, max_pos = self.servo_limits[servo_id]
                safe_pos = max(min_pos, min(max_pos, position))
                targets.append((servo_id, safe_pos))
                self.current_positions[servo_id] = safe_pos
            
            self.driver.set_servo_positions(targets, time_ms)
            angle = self.position_to_angle(position)
            print(f"✓ 舵机移动完成 (目标角度 {angle:.1f}°)")
            for servo_id, pos in targets:
                a = self.position_to_angle(pos)
                print(f"  舵机 {servo_id}: 位置 {pos} (角度 {a:.1f}°)")
    ...
    def main_menu(self):
        ...
        while True:
            print(f"\n{'='*60}")
            print(f"Delta 机械臂 - LX-225 舵机调试工具")
            print(f"{'='*60}")
            print("选项:")
